            ...
            <div class="card">
              ...
              <div class="card-body">
                <table id="table1" class="table table-bordered table-hover">
                  ...
                  <tbody>
                        <?php
                           $sql_pembayaran = mysqli_query($conn, "SELECT * FROM tb_pmbyranPelanggan") or die(mysqli_error($conn));
                           while($pembayaran = mysqli_fetch_array($sql_pembayaran)){
                              $sql_pelanggan = mysqli_query($conn, "SELECT * FROM tb_pelanggan WHERE id_pelanggan = '".$pembayaran['id_pelanggan']."'");
                              $pelanggan = mysqli_fetch_array($sql_pelanggan);
                        ?>
                        <tr>
                           <td><?php echo $pembayaran['tanggal'] ?></td>
                           <td><?php echo $pembayaran['invoice_penjualan'] ?></td>
                           <td><?php echo $pelanggan['nama_pelanggan'] ?></td>
                           <td><?php echo $pelanggan['daerah'] ?></td>
                           <td><?php echo $pembayaran['nama_salesman'] ?></td>
                           <td><?php echo rupiah($pembayaran['jumlah_pembayaran']) ?></td>
                           <td>
                              <a href="javascript:deletePembayaran(`<?php echo $pembayaran['invoice_pembayaran'] ?>`)" class="btn btn-danger btn-sm"><i class="fas fa-trash-alt"></i></a>
                           </td>
                        </tr>
                        <?php } ?>
                  </tbody>
...
function deletePembayaran(id){
   Swal.fire({
     ...
     if (result.isConfirmed) {
        window.location.href='deletePembayaranPelanggan.php?id='+id;
       Swal.fire(
         'Deleted!',
