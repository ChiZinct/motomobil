...
<div class="wrapper">
  ...
  <div class="content-wrapper">
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Tambah Pembelian Barang</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
    ...
    <!-- Main content -->
    <?php
    $id   = mysqli_query($conn, "SELECT MAX(MID(no_invoice, 19, 4)) as invoice_no FROM tb_pembelian
    WHERE MID(no_invoice, 12, 6) = DATE_FORMAT(CURDATE(), '%y%m%d') AND MID(no_invoice, 8, 3) = '".$login_as['id_gudang']."'");
   ...
   $prefix = "MD/PBL/".$login_as['id_gudang']."/".date("ymd")."/";
                ...
                <div class="form-horizontal">
                  <div class="card-body">
                    ...
                    <div class="form-group row">
                      ...
                      <div class="col-sm-7">
                        <input type="text" class="form-control" id="no_plat" name="no_plat" placeholder="ex: A-1111-AA">
                      </div>
                    </div>
                  </div>
               </div>
            ...
            <div class="card">
              <div class="card-body">
                <table id="tablePembelian" class="table table-bordered table-hover">
                  ...
                  <tbody>
                     <?php
                     $sql_pembelian = mysqli_query($conn, 'SELECT * FROM `tb_transaksi` WHERE `no_invoice` = \''.mysqli_escape_string($conn, $no_invoice).'\'') or die(mysqli_error($conn));
                     while($pembelian =  mysqli_fetch_array($sql_pembelian)) {
                    ...
                    <tr>
                      ...
                      <td><?php echo ribukoma($pembelian['qty']) ?></td>
                      <td><?php echo $pembelian['satuan'] ?></td>
                      <td><?php echo rupiah($barang['harga_beli'])?></td>
                      <td><?php echo rupiah($pembelian['total_harga_satuan']) ?></td>
                      <td><a href="javascript:deleteTransaksi(`<?php echo $pembelian['id_barang'].$pembelian['no_invoice']?>`)" class="btn btn-danger btn-sm">
    ...
    $('#submit').click(function(e){
      ...
      var valid = this.form.checkValidity();
        ...
        if(valid){
          ...
          $.ajax({
              type: 'POST',
              url: 'submitPembelian.php',
              data: {no_invoice: no_invoice, tanggal: tanggal, supplier: supplier, no_plat: no_plat},
              success: function(data){
                 Swal.fire({
                  ...
                  confirmButtonText: `<a style="color: white;" href="../dataTransaksiBarang/tablePembelianBarang.php">Go To Table Pembelian Barang</a>`,
                  denyButtonText: `<a style="color: white;" href="pembelianBarang.php">Insert Another Form</a>`,
                  })
              },
              error: function(data){
                    Swal.fire({
