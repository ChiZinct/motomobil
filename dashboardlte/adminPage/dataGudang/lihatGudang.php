...
<div class="wrapper">
    ...
    <div class="sidebar">
      ...
      <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="true">
          ...
          <!-- Data Gudang -->
          <li class="nav-item">
            <a href="tableGudang.php" class="nav-link active">
  ...
  <div class="content-wrapper">
    ...
    <!-- Main content -->
    <?php
    $query_gudang = mysqli_query($conn, "SELECT * FROM tb_user WHERE id_gudang = '".$_GET['id']."'");
    $gudang = mysqli_fetch_array($query_gudang);
     ?>
              ...
              <div class="card-header">
                <h3 class="card-title">
                  ...
                  if($_GET['id'] === "UTM") {
                     ...
                     echo "Gudang ".$gudang['nama_depan']." ".$gudang['nama_belakang'];
                  }
                  ?>
                </h3>
              </div>
                ...
                <table id="table1" class="table table-bordered table-hover">
                  ...
                  <tbody>
                     ...
                     while($barang = mysqli_fetch_array($query_barang)) {
                        if($_GET['id'] === "UTM") {
                           //query perhitungan pembelian
                           $query_pembelian = mysqli_query($conn, "SELECT SUM(qty) AS qty FROM tb_transaksi WHERE MID(no_invoice, 4, 3) = 'PBL' AND id_barang = '".$barang['id_barang']."'");
                           //query perhitungan barang masuk
                           $query_barangMasuk = mysqli_query($conn, "SELECT SUM(a.qty) AS qty, b.* FROM tb_transaksi a, tb_barangmasuk b WHERE (b.masuk_ke = '".$_GET['id']."' AND a.no_invoice = b.no_invoice) AND a.id_barang = '".$barang['id_barang']."'");
                           //query perhitungan barang keluar
                           $query_barangKeluar = mysqli_query($conn, "SELECT SUM(a.qty) AS qty, b.* FROM tb_transaksi a, tb_barangkeluar b WHERE (b.keluar_dari = '".$_GET['id']."' AND a.no_invoice = b.no_invoice) AND a.id_barang = '".$barang['id_barang']."'");
                           //query perhitungan transfer barang dari gudang utama
                           $query_transferDari = mysqli_query($conn, "SELECT SUM(a.qty) AS qty, b.* FROM tb_transaksi a, tb_transferbarang b WHERE (b.gudang_asal = '".$_GET['id']."' AND a.no_invoice = b.no_invoice) AND a.id_barang = '".$barang['id_barang']."'");
                           //query perhitungan transfer barang ke gudang utama
                           $query_transferKe = mysqli_query($conn, "SELECT SUM(a.qty) AS qty, b.* FROM tb_transaksi a, tb_transferbarang b WHERE (b.gudang_tujuan = '".$_GET['id']."' AND a.no_invoice = b.no_invoice) AND a.id_barang ='".$barang['id_barang']."'");
                           $pembelian = mysqli_fetch_array($query_pembelian);
                         ...
                         ?>
                           <tr>
                              ...
                              <td>
                                 <?php
                                  $total = ($pembelian['qty'] + $barang_masuk['qty'] + $transfer_ke['qty']) - ($barang_keluar['qty'] + $transfer_dari['qty']);
                                  echo ribukoma($total)." ".$barang['satuan_jual'];
                                   ?>
                           </td>
                        </tr>
                     <?php
                        } elseif(substr($_GET['id'], 0, 2) === "G-") {
                           $query_salesman = mysqli_query($conn, "SELECT * FROM tb_user WHERE id_gudang = '".$_GET['id']."'");
                           $salesman = mysqli_fetch_array($query_salesman);
                           $nama_salesman = $salesman['nama_depan']." ".$salesman['nama_belakang'];
                           //query perhitungan barang masuk
                           $query_barangMasuk = mysqli_query($conn, "SELECT SUM(a.qty) AS qty, b.* FROM tb_transaksi a, tb_barangmasuk b WHERE (b.masuk_ke = '".$_GET['id']."' AND a.no_invoice = b.no_invoice) AND a.id_barang = '".$barang['id_barang']."'");
                           //query perhitungan barang keluar
                           $query_barangKeluar = mysqli_query($conn, "SELECT SUM(a.qty) AS qty, b.* FROM tb_transaksi a, tb_barangkeluar b WHERE (b.keluar_dari = '".$_GET['id']."' AND a.no_invoice = b.no_invoice) AND a.id_barang = '".$barang['id_barang']."'");
